...
namespace GenCode.DroneDown.Android
{
    ...
    public class MainActivity : AndroidActivity, IBeaconConsumer
    {
        ...
        public MainActivity()
        {
            try
            {
                MessagingCenter.Subscribe<TabPageWelcome, BeaconDevice>(this, "DeviceChanged", (sender, messageData) => {
...
#pragma warning restore 219
                ...
                // setting up a beacon by passing in any BeaconDevice you want
                BeaconDevices.Default.GetBeaconDevice = new BeaconDevices.Manufacturers.Bkon().GetBeaconDevice;
                SetupBeacons(BeaconDevices.Default.GetBeaconDevice);
        ...
        /// Setups your beacon.
        /// </summary>
        /// <param name="beaconDevice">manufucturer.</param>
        private void SetupBeacons(BeaconDevice beaconDevice)
        {
            try
            {
                if (beaconDevice != null)
                {
                    ...
                    _monitoringRegion = new Region(beaconDevice.BeaconId, beaconDevice.Uuid, null, null);
                    _rangingRegion = new Region(beaconDevice.BeaconId, beaconDevice.Uuid, null, null);
        ...
        void RangingBeaconsInRegion(object sender, RangeEventArgs e)
        {
            try
            {
                ...
                // log whats happening
                Log.WriteLine(String.Format("RangingBeaconsInRegion for {0} sending message rssi {1} accuracy {2}", BeaconDevices.Default.GetBeaconDevice.BeaconId, rssi, accuracy), TraceLogLevel.Verbose);
            }
            catch (Exception ex)
            {
                Log.WriteLine(ex);
